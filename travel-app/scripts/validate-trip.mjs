#!/usr/bin/env node
// Schema gate for every trip registered in src/trips/index.js. Run
// `npm run validate` before build/deploy. Pass --images to additionally
// HEAD-check every image URL (needs network).
...
let totalSpots = 0, totalStops = 0, totalDays = 0

function validateBundle({ CATEGORIES, CATEGORY_IMAGES, STOPS, SPOTS, HOTELS, trip }) {
  const tag = trip?.slug || '(no slug)'
  const err = m => errors.push(`[${tag}] ${m}`)
  const warn = m => warns.push(`[${tag}] ${m}`)
  ...
  totalSpots += SPOTS.length; totalStops += STOPS.length; totalDays += days.length
}

// slugs must be unique across trips — they drive routing
const slugs = tripBundles.map(b => b.trip.slug)
slugs.forEach((s, i) => { if (slugs.indexOf(s) !== i) errors.push(`duplicate trip slug "${s}" in registry`) })

tripBundles.forEach(validateBundle)

async function checkImages() {
  const urls = new Set()
  tripBundles.forEach(({ STOPS, SPOTS, CATEGORY_IMAGES, trip }) => {
  ...
  await Promise.all([...urls].map(async u => {
    try {
      ...
      if (!r.ok) errors.push(`image ${r.status}: ${u}`)
    } catch { errors.push(`image unreachable: ${u}`) }
  }))
}
...
const run = async () => {
  ...
  if (errors.length) {
    ...
    console.error(`\n${errors.length} error(s). Trip data is NOT schema-valid.`)
    process.exit(1)
  }
  console.log(`✅ ${tripBundles.length} trip(s) schema-valid (${totalSpots} spots, ${totalStops} stops, ${totalDays} days${warns.length ? `, ${warns.length} warning(s)` : ''}).`)
}
run()
